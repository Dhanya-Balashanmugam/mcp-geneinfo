...
def search_uniprot(keyword, limit=10):
...
def get_ensembl_gene_info_by_symbol(gene_symbol):
        ...
        for i, gene in enumerate(genes, 1):
            with st.expander(f"{i}. {gene['gene']} | UniProt ID: {gene['uniprot_id']}"):
                ...
                with st.spinner(f"Fetching Ensembl info for {gene['gene']}..."):
                    ensembl_info = get_ensembl_gene_info_by_symbol(gene['gene'])
